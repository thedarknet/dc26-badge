...
{
	char * msg = nullptr;
	if (iUartServerCallbacks.isConnected) // FIXME: could have stale entries in queue
	{
		if(xQueueReceive(CallbackQueueHandle, &msg, CbackQTimeout) ==
			pdTRUE)
		{
...
void BluetoothTask::commandHandler(MCUToMCUTask::Message* msg)
{
	...
	switch (msg->Msg_type())
	{
		...
		case : // set device name
			break;
		case : // get infection data
			// send infection data
			break;
		case : // set infection data
			break;
		case : // scan for badges
			// send results
			break;
		case : // scan for NPCs
			// send results
			break;
		case : // connect to address 
			break;
		case : // disconnect from address
			break;
		case : // send data to address
			break;
		default :
			// send failure
	}
